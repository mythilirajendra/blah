...
def main_func(suite, make_html, errors_and_issues):
    ...
    if not errors_and_issues['source'] and tree.get_element_attribute(suite, 'source') is not None:
        errors_and_issues['source'] =  '-'.join(re.split(r' |/|\\' , tree.get_element_attribute(suite, 'source'))[-3:-1])
    if not errors_and_issues['version'] and tree.get_elements(suite, 'metadata'):
        errors_and_issues['version'] = tree.get_element_text(tree.get_elements(suite, 'metadata')[0])
    if is_failed(suite):
        ...
        errors_and_issues['pass'] += len(tree.get_elements(suite, 'test'))
        if make_html=='y':
            clear_suite(suite)
...
def get_stats(i_e):
    ...
    if len(i_e) == 1:
        print(i_e)
        exit()
    count = {'environment issue':[0,0,0],'automation issue':[0,0,0],'product issue':[0,0,0], 'pass':0, 'fail':0}
    total = 0
    for e_i in i_e:
        count['pass'] += e_i['pass']
        count['fail'] += e_i['fail']
...
def get_info(root, e_i):
    ...
    for x in tree.get_elements(root, 'suite'):
        try:
            e_i['version'] = tree.get_element_text(tree.get_child_elements(x, 'metadata')[0])
        except:
            pass
        try:
            source = tree.get_element_attribute(x, 'source') #no need of try?
            if source is not None:
                e_i['source'] =  '-'.join(re.split(r' |/|\\' , source)[-3:-1])
        except:
            pass
        if not e_i['version']and not e_i['source']:
            break
...
def for_url(url, i_e, exclude_list, strt_date, end_date, make_html, dest, make_report):
    ...
    if url.endswith('.xml'):
        ...
        with urlopen(url) as f:
            root = tree.parse_xml(f)
            # p = etree.XMLParser(huge_tree=True)
            # lxmltree = etree.parse(url, parser=p)
            # root = lxmltree.getroot()
            main_suite = tree.get_element(root, 'suite')
        print(datetime.datetime.now().strftime("%H:%M:%S"))
        # print(datetime.datetime.now().strftime("%H:%M:%S"))
        # context = etree.iterparse(x.content, huge_tree=True) 
        # for event, element in context:
        #     if element == 'suite':
        #         print(tree.get_element_attributes(element))
        #         break
        #     element.clear()
        # print(datetime.datetime.now().strftime("%H:%M:%S"))
        ...
        if not is_failed(main_suite):
            ...
            e_i = main_func(main_suite, make_html, {'version': '', 'source': '', 'pass': 0, 'fail': 0})
            if make_html == 'y':
                dirs = re.split('.net' , url)[1]
                new_logfile = get_path(dest, dirs)
...
def for_file(xml_file, make_html, dest, path, make_report):
    fd = open(xml_file, 'r')
    try:
    ...
    if not is_failed(main_suite):
        ...
        e_i = main_func(main_suite, make_html, {'version': '', 'source': '', 'pass': 0, 'fail': 0})
        if make_html=='y':
            new_logfile = get_path(dest, path)
            create_html(new_logfile, root, e_i)
            if dest[0:4] == 'http':
                upload_to_url(new_logfile[:-3] + 'html', dest, path)
                os.remove(new_logfile[:-3] + 'html')
                os.remove(new_logfile)
    return e_i
        ...
        if each_input[0:4] == "http":
            ...
            for xml_file in (x for x in files if (x not in exclude_list and not any(exclude in x for exclude in exclude_list))):
                ...
                if is_in_duration(dates[0], date_mod, dates[1]):
                    ...
                    e_i = for_file(xml_file, make_html, dest, path, make_report)
                    if e_i is not None:
                        i_e.append(e_i)
                    # except AssertionError as e:
                    #     print("XML file structure issue. Suite wasn't found")
        else:
            e_i = for_file(each_input, make_html, dest, re.split(r' |/|\\' , each_input)[-1] , make_report) #os.path.join(dest,re.split(r' |/|\\' , each_input)[-1])
            if e_i is not None:
                i_e.append(e_i)
